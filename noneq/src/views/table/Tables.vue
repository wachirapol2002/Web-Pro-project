      ...
      <div class="grid">
        <div :class="center" v-for="table in tables" :key="table.table_num">
          <div
            ...
            :class="{
              'bg-primary':
                table.table_status == 'unavailable' &&
                table.username == account.username,
              'bg-success': table.table_status == 'available',
              'bg-warning':
                (table.table_status == 'booked' &&
                  table.username == account.username) ||
                (table.table_status == 'booked' &&
                  account.permission == 'staff'),
              'bg-danger':
                (table.table_status == 'unavailable' &&
                  table.username != account.username) ||
                (table.table_status == 'booked' &&
...
export default {
  ...
  methods: {
    ...
    async checkhaveTable() {
      ...
      axios
        .post("http://44.206.165.83:3000/table/haveTable", data)
        .then((response) => {
    ...
    selectTable(tableNum) {
      if (this.$cookies.isKey("account")) {
        if (this.tables[tableNum - 1].table_status == "available") {
          //โต๊ะว่าง
          if (this.account.permission == "staff") {
            // staff
            this.$router.push({
              path: "/tables/checkin",
              query: { table: tableNum },
            });
          } else if (this.account.permission == "VIP") {
            //VIP
        ...
        } else if (this.tables[tableNum - 1].table_status == "booked") {
          //ถูกจอง
          if (this.account.permission == "staff") {
            // staff
            this.$router.push({
            ...
            //VIP
            if (this.tables[tableNum - 1].username == this.account.username) {
              //เจ้าของโต๊ะ
              this.$router.push({
                path: "/tables/checkin",
                query: { table: tableNum },
          ...
          if (this.account.permission == "staff") {
            ...
            this.account.username == this.tables[tableNum - 1].username
          ) {
            //เจ้าของโต๊ะ
            this.$router.push({ path: "/order", query: { table: tableNum } });
          } else {
